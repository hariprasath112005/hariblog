...
3. You can see the flag which has `INFO FLAGPART:` 
4. We need to filter the results with `grep` tool and only filter the lines which has `INFO FLAGPART` in it.
5. `cat server.log | grep 'INFO FLAGPART:'
